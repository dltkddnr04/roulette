import type { MapEntity } from '../types/MapEntity.type';
import type { VectorLike } from '../types/VectorLike';

export type BrandingBoard = {
  x: number;
  y: number;
  w?: number;
  h?: number;
};

export type MarbleSpawnDefinition = {
  /** Position of the first marble in the bottom row, before row shifting. */
  origin: VectorLike;
  maxColumns: number;
  columnSpacing: number;
  rowSpacing: number;
  /** Number of rows that can spawn before the stack shifts down. */
  maxUnshiftedRows: number;
};

export const DEFAULT_MARBLE_SPAWN: MarbleSpawnDefinition = {
  origin: { x: 10.25, y: 1 },
  maxColumns: 10,
  columnSpacing: 0.6,
  rowSpacing: 1,
  maxUnshiftedRows: 5,
};

export type StageDef = {
  title: string;
  entities?: MapEntity[];
  finish: {
    y: number;
  };
  camera: {
    zoomTriggerY: number;
  };
  spawn: MarbleSpawnDefinition;
  branding?: BrandingBoard[];
};

export const stages: StageDef[] = [
  {
    title: 'Wheel of fortune',
    finish: { y: 111 },
    camera: { zoomTriggerY: 106.75 },
    spawn: DEFAULT_MARBLE_SPAWN,
    branding: [{ x: 15.92, y: 104.3, w: 5, h: 1.25 }],
    entities: [
      // polyline
      {
        position: { x: 0, y: 0 },
        shape: {
          type: 'polyline',
          points: [
            [16.5, -300],
            [9.25, -300],
            [9.25, 8.5],
            [2, 19.25],
            [2, 26],
            [9.75, 30],
            [9.75, 33.5],
            [1.25, 41],
            [1.25, 53.75],
            [8.25, 58.75],
            [8.25, 63],
            [9.25, 64],
            [8.25, 65],
            [8.25, 99.25],
            [15.1, 106.75],
            [15.1, 111.75],
          ],
        },
        type: 'static',
        props: { restitution: 0 },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [16.5, -300],
            [16.5, 9.25],
            [9.5, 20],
            [9.5, 22.5],
            [17.5, 26],
            [17.5, 33.5],
            [24, 38.5],
            [19, 45.5],
            [19, 55.5],
            [24, 59.25],
            [24, 63],
            [23, 64],
            [24, 65],
            [24, 100.5],
            [16, 106.75],
            [16, 111.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [12.75, 37.5],
            [7, 43.5],
            [7, 49.75],
            [12.75, 53.75],
            [12.75, 37.5],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [14.75, 37.5],
            [14.75, 43],
            [17.5, 40.25],
            [14.75, 37.5],
          ],
        },
      },
      // boxes
      {
        position: { x: 15.5, y: 30.0 },
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4 },
        type: 'static',
        props: { restitution: 1 },
      },
      {
        position: { x: 15.5, y: 32 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.5, y: 28 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 12.5, y: 30 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 12.5, y: 32 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 12.5, y: 28 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.4, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 11.3, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 13.2, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.1, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 17, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 18.9, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 20.699999999999996, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 22.7, y: 66.6 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.4, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 11.3, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 13.2, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.1, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 17, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 18.9, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 20.699999999999996, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 22.7, y: 69.1 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.6, halfHeight: 0.1, rotation: -Math.PI / 4 },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.5, y: 92 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 12.75, y: 92 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 16, y: 92 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.25, y: 92 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 22.5, y: 92 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 11, y: 95 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 14.25, y: 95 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 17.5, y: 95 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 20.75, y: 95 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.5, y: 98 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 12.75, y: 98 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 16, y: 98 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.25, y: 98 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 22.5, y: 98 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 0.25,
          halfHeight: 0.25,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },

      // wheels
      {
        position: { x: 8, y: 75 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 3.5, restitution: 0 },
      },
      {
        position: { x: 12, y: 75 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -3.5, restitution: 0 },
      },
      {
        position: { x: 16, y: 75 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 3.5, restitution: 0 },
      },
      {
        position: { x: 20, y: 75 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -3.5, restitution: 0 },
      },
      {
        position: { x: 24, y: 75 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 3.5, restitution: 0 },
      },
      {
        position: { x: 14, y: 106.75 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -1.2, restitution: 0 },
      },
    ],
  },
  {
    title: 'BubblePop',
    finish: { y: 83 },
    camera: { zoomTriggerY: 78 },
    spawn: DEFAULT_MARBLE_SPAWN,
    branding: [
      { y: 80.5, x: 10 },
      { y: 80.5, x: 15.05 },
    ],
    entities: [
      {
        type: 'static',
        position: { x: 10.375, y: -108.5 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [6.125, -191.5],
            [-1.125, -191.5],
            [-1.125, 108.5],
            [-1.125, 151.5],
            [-6.125, 158.5],
            [-1.125, 161.5],
            [-1.125, 179.5],
            [-0.9128679656440362, 179.7498817789222],
            [-1.125, 179.9997635578444],
            [-1.125, 183.5],
            [1.625, 188.5],
            [1.625, 191.5],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 16.25, y: -108.5 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [0.25, -191.5],
            [0.25, 158.5],
            [3.25, 162.5],
            [2.25, 164.5],
            [3.25, 166.5],
            [0.25, 169.5],
            [0.25, 179.5],
            [0.03786796564403616, 179.75925677892224],
            [0.25, 179.9997635578444],
            [0.25, 183.5],
            [-3.25, 188.5],
            [-3.25, 191.5],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 16.5, y: 55.75 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [0, -3.25],
            [1, -1.75],
            [0, 0.25],
            [1, 2.25],
            [0, 3.25],
            [-1, 0.25],
            [0, -3.25],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 10.375, y: 48.25 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [-1.125, -2.75],
            [-4.125, 1.25],
            [-1.125, 2.75],
            [4.125, 2.25],
            [-1.125, -2.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 10.15625, y: 26.75 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [-0.90625, -0.75],
            [0.90625, 0.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 15.59375, y: 26.75 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [0.90625, -0.75],
            [-0.90625, 0.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 12.875, y: 29.25 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [-1.8125, 0.75],
            [0, -0.75],
            [1.8125, 0.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 10.15625, y: 31.75 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [-0.90625, -0.75],
            [0.90625, 0.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 15.59375, y: 31.75 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [0.90625, -0.75],
            [-0.90625, 0.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 12.875, y: 34.25 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [-1.8125, 0.75],
            [0, -0.75],
            [1.8125, 0.75],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 9.25, y: 18 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 11.25, y: 18 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 13.25, y: 18 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 15.25, y: 18 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 10.5, y: 19 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 12.5, y: 19 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 14.5, y: 19 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 16.5, y: 19 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 9.25, y: 20 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 11.25, y: 20 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 13.25, y: 20 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 15.25, y: 20 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 10.5, y: 21 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 12.5, y: 21 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 14.5, y: 21 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 16.5, y: 21 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 9.25, y: 22 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 11.25, y: 22 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 13.25, y: 22 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 15.25, y: 22 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 10.5, y: 23 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 12.5, y: 23 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 14.5, y: 23 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 16.5, y: 23 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 0.15, halfHeight: 0.15 },
      },
      {
        type: 'static',
        position: { x: 9.400000000000002, y: 39 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 3, halfHeight: 3 },
      },
      {
        type: 'static',
        position: { x: 16.5, y: 43 },
        props: { restitution: 0 },
        shape: { type: 'box', rotation: 0.7853981633974483, halfWidth: 3, halfHeight: 3 },
      },
      {
        type: 'kinematic',
        position: { x: 10.7, y: 10 },
        props: { angularVelocity: 10, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 0.5, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 14.7, y: 10 },
        props: { angularVelocity: -10, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 0.5, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 12.7, y: 10 },
        props: { angularVelocity: 10, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 0.5, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 10.7, y: 14 },
        props: { angularVelocity: -3, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 2, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 14.7, y: 14 },
        props: { angularVelocity: 3, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 2, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 11.2, y: 44 },
        props: { angularVelocity: -5, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 0.5, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 10.3, y: 75 },
        props: { angularVelocity: 8, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 1, halfHeight: 0.1 },
      },
      {
        type: 'kinematic',
        position: { x: 15.462132034355964, y: 75 },
        props: { angularVelocity: -8, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 1, halfHeight: 0.1 },
      },
      {
        type: 'static',
        position: { x: 11, y: 65 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 13, y: 65 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 15, y: 65 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 12, y: 67.5 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 14, y: 67.5 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 13, y: 69.77058813837772 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.8 },
      },
      {
        type: 'static',
        position: { x: 10.7, y: 77.5 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.8 },
      },
      {
        type: 'static',
        position: { x: 14.7, y: 77.5 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.8 },
      },
      {
        type: 'static',
        position: { x: 12.625, y: 80 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 1.5 },
      },
      {
        type: 'static',
        position: { x: 12.625, y: 80 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 1.2 },
      },
      {
        type: 'kinematic',
        position: { x: 12.625, y: 56.00000000000001 },
        props: { angularVelocity: -8, restitution: 0 },
        shape: { type: 'box', rotation: 0, halfWidth: 1, halfHeight: 0.1 },
      },
      {
        type: 'static',
        position: { x: 9.947604593262161, y: 62.59581680393866 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 11.947604593262161, y: 62.59581680393866 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 13.947604593262161, y: 62.59581680393866 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
      {
        type: 'static',
        position: { x: 15.828283102570442, y: 62.59581680393866 },
        props: { restitution: 1.5, destroyOnContact: true },
        shape: { type: 'circle', radius: 0.5 },
      },
    ],
  },
  {
    title: 'Pot of greed',
    finish: { y: 91 },
    camera: { zoomTriggerY: 92.5 },
    spawn: DEFAULT_MARBLE_SPAWN,
    branding: [{ x: 13.0, y: 75.5, w: 9.5, h: 2.375 }],
    entities: [
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [17, -300],
            [9, -300],
            [9, 8.5],
            [2, 15],
            [6, 61.5],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [7, 67.2],
            [9, 85.2],
            [8, 84.9],
            [6, 84.6],
            [5, 78.6],
            [4, 66.6],
            [7, 67.2],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [17, -300],
            [17, 8.5],
            [24, 15],
            [20, 61.5],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [19, 67.2],
            [17, 85.2],
            [18, 84.9],
            [20, 84.6],
            [21, 78.6],
            [22, 66.6],
            [19, 67.2],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [11, 77.4],
            [12, 78.6],
            [12, 91.8],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [15, 77.4],
            [14, 78.6],
            [14, 91.8],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [12, 85.8],
            [11, 86.4],
            [9, 87],
            [8, 87],
            [6, 86.4],
            [5, 85.8],
            [4, 84.6],
            [3, 78.6],
            [2, 66.6],
            [3, 63.6],
            [4, 62.4],
            [5, 61.8],
            [6, 61.5],
          ],
        },
      },
      {
        type: 'static',
        position: { x: 0, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'polyline',
          points: [
            [14, 85.8],
            [15, 86.4],
            [17, 87],
            [18, 87],
            [20, 86.4],
            [21, 85.8],
            [22, 84.6],
            [23, 78.6],
            [24, 66.6],
            [23, 63.6],
            [22, 62.4],
            [21, 61.8],
            [20, 61.5],
          ],
        },
      },
      {
        position: { x: 13, y: 20 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 3,
          halfHeight: 3,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 13, y: 55 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 3,
          halfHeight: 3,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 8, y: 37 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 2,
          halfHeight: 2,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 18, y: 37 },
        type: 'static',
        shape: {
          type: 'box',
          halfWidth: 2,
          halfHeight: 2,
          rotation: 0.7853981633974483,
        },
        props: { restitution: 0 },
      },
      {
        position: { x: 11, y: 12 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -3, restitution: 0 },
      },
      {
        position: { x: 15, y: 12 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 3, restitution: 0 },
      },
      {
        position: { x: 8, y: 87 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 1, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 6, y: 86.4 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 1.5, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 4, y: 84.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 1.5, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 3.5, y: 81.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 3, y: 78.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 2.75, y: 75.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 2.5, y: 72.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 2.25, y: 69.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 2, y: 66.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: -10, restitution: 0 },
      },
      {
        position: { x: 18, y: 87 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 1, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 20, y: 86.4 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 1.5, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 22, y: 84.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 1.5, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 22.5, y: 81.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 23, y: 78.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 23.25, y: 75.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 23.5, y: 72.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 23.75, y: 69.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
      {
        position: { x: 24, y: 66.6 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0 },
        props: { angularVelocity: 10, restitution: 0 },
      },
    ],
  },
  {
    title: 'Yoru ni Kakeru',
    finish: { y: 248 },
    camera: { zoomTriggerY: 234.5 },
    spawn: DEFAULT_MARBLE_SPAWN,
    branding: [
      { x: 11.5, y: 12, w: 16, h: 4 },
      { x: 11.5, y: 155, w: 16, h: 4 },
    ],
    entities: [
      {
        position: { x: 2, y: 0 },
        shape: {
          type: 'box',
          halfWidth: 1,
          halfHeight: 800,
          rotation: 0,
          color: '#222',
          bloomColor: '#777',
        },
        type: 'static',
        props: { restitution: 0 },
      },
      {
        type: 'static',
        position: { x: 21, y: 0 },
        props: { restitution: 0 },
        shape: {
          type: 'box',
          rotation: 0,
          halfWidth: 1,
          halfHeight: 800,
          color: '#222',
          bloomColor: '#777',
        },
      },
      {
        position: { x: 4.0, y: 25.0 },
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        type: 'static',
        props: { restitution: 1 },
      },
      {
        position: { x: 4.0, y: 30.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 4.0, y: 35.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 4.0, y: 40.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.0, y: 25.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.0, y: 30.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.0, y: 35.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.0, y: 40.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 14.0, y: 25.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 14.0, y: 30.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 14.0, y: 35.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 14.0, y: 40.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.0, y: 25.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.0, y: 30.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.0, y: 35.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.0, y: 40.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.2, halfHeight: 0.2, rotation: -Math.PI / 4, color: '#818fb4' },
        props: { restitution: 0 },
      },

      {
        position: { x: 6.5, y: 27.5 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0, color: '#9bec00' },
        props: { angularVelocity: 2.0, restitution: 0 },
      },
      {
        position: { x: 6.5, y: 37.5 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0, color: '#ff6868' },
        props: { angularVelocity: 2.0, restitution: 0 },
      },
      {
        position: { x: 11.5, y: 32.5 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0, color: '#80b3ff' },
        props: { angularVelocity: 4.0, restitution: 0 },
      },
      {
        position: { x: 16.5, y: 27.5 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0, color: '#ff6868' },
        props: { angularVelocity: -2.0, restitution: 0 },
      },
      {
        position: { x: 16.5, y: 37.5 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 2, halfHeight: 0.1, rotation: 0, color: '#9bec00' },
        props: { angularVelocity: -2.0, restitution: 0 },
      },

      {
        position: { x: 10.0, y: 26.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 26.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.0, y: 26.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.0, y: 27.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 27.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.0, y: 27.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.0, y: 29.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 29.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.0, y: 29.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 5.0, y: 31.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 31.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 31.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.0, y: 32.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 32.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 32.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.0, y: 34.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 34.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 34.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 15.0, y: 31.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 31.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.0, y: 31.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.0, y: 32.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 32.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.0, y: 32.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.0, y: 34.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 34.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.0, y: 34.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 10.0, y: 36.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 36.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.0, y: 36.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.0, y: 37.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 37.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.0, y: 37.5 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.0, y: 39.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 39.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.0, y: 39.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 11.5, y: 50.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 6.5, halfHeight: 0.1, rotation: 0, color: '#5c5470' },
        props: { angularVelocity: -2.0, restitution: 0 },
      },

      {
        position: { x: 3.5, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.0, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.0, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.0, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.0, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 20.0, y: 60.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.0, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.0, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.0, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.0, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.0, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.0, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 63.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe227' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.5, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.0, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.0, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.0, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.0, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 20.0, y: 66.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#fff4b7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 4.5, y: 75.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 4.0, halfHeight: 0.1, rotation: 0, color: '#ff577f' },
        props: { angularVelocity: -4.0, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 75.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 4.0, halfHeight: 0.1, rotation: 0, color: '#ff577f' },
        props: { angularVelocity: 4.0, restitution: 0 },
      },

      {
        position: { x: 3.8, y: 90.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.2, y: 90.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 5.8, y: 92.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 17.2, y: 92.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 7.8, y: 94.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.2, y: 94.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.8, y: 96.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 13.2, y: 96.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },

      {
        position: { x: 3.8, y: 94.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.2, y: 94.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 5.8, y: 96.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 17.2, y: 96.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 7.8, y: 98.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.2, y: 98.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.8, y: 100.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 13.2, y: 100.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },

      {
        position: { x: 10.0, y: 90.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.0, y: 90.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 92.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.0, y: 92.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.0, y: 94.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.0, y: 94.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.0, y: 96.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.0, y: 96.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ffe3fe' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 10.0, y: 94.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.0, y: 94.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.0, y: 96.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.0, y: 96.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.0, y: 98.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.0, y: 98.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.0, y: 100.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.0, y: 100.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#ff94cc' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 4.5, y: 100.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 5.0, halfHeight: 0.1, rotation: 0, color: '#5c8374' },
        props: { angularVelocity: -2.5, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 100.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 5.0, halfHeight: 0.1, rotation: 0, color: '#5c8374' },
        props: { angularVelocity: 2.5, restitution: 0 },
      },

      {
        position: { x: 3.8, y: 104.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.2, y: 104.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 5.8, y: 106.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 17.2, y: 106.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 7.8, y: 108.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.2, y: 108.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.8, y: 110.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },
      {
        position: { x: 13.2, y: 110.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#c7ffd8' },
        props: { restitution: 0 },
      },

      {
        position: { x: 3.8, y: 108.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 19.2, y: 108.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 5.8, y: 110.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 17.2, y: 110.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 7.8, y: 112.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 15.2, y: 112.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 9.8, y: 114.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },
      {
        position: { x: 13.2, y: 114.0 },
        type: 'static',
        shape: { type: 'box', halfWidth: 0.1, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#98ded9' },
        props: { restitution: 0 },
      },

      {
        position: { x: 3.5, y: 105.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 105.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 3.5, y: 107.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 107.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 107.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 107.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 3.5, y: 109.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 109.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 109.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 109.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 109.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 109.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.5, color: '#edeef7' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 5.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.4, color: '#e6176d' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.4, color: '#e6176d' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 1.2, color: '#e64588' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 1.2, color: '#e64588' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 2.0, color: '#e673a3' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 2.0, color: '#e673a3' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 2.8, color: '#e6a1bd' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 2.8, color: '#e6a1bd' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 3.8, color: '#e6cfd8' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 115.0 },
        type: 'static',
        shape: { type: 'circle', radius: 3.8, color: '#e6cfd8' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 11.5, y: 120.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.4, color: '#e6176d' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 120.0 },
        type: 'static',
        shape: { type: 'circle', radius: 1.2, color: '#e64588' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 120.0 },
        type: 'static',
        shape: { type: 'circle', radius: 2.0, color: '#e673a3' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 120.0 },
        type: 'static',
        shape: { type: 'circle', radius: 2.8, color: '#e6a1bd' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 120.0 },
        type: 'static',
        shape: { type: 'circle', radius: 3.8, color: '#e6cfd8' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 4.5, y: 130.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 5.0, halfHeight: 0.1, rotation: 0, color: '#435585' },
        props: { angularVelocity: 0.25, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 130.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 5.0, halfHeight: 0.1, rotation: 0, color: '#5c8374' },
        props: { angularVelocity: -0.32, restitution: 0 },
      },
      {
        position: { x: 4.5, y: 140.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 4.0, halfHeight: 0.1, rotation: Math.PI / 6, color: '#610c9f' },
        props: { angularVelocity: 0.32, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 140.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 4.0, halfHeight: 0.1, rotation: -Math.PI / 6, color: '#872341' },
        props: { angularVelocity: -0.32, restitution: 0 },
      },
      {
        position: { x: 4.5, y: 150.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 5.0, halfHeight: 0.1, rotation: Math.PI / 3, color: '#503c3c' },
        props: { angularVelocity: 0.32, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 150.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 5.0, halfHeight: 0.1, rotation: -Math.PI / 3, color: '#5c5470' },
        props: { angularVelocity: -0.32, restitution: 0 },
      },
      {
        position: { x: 4.5, y: 160.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 4.0, halfHeight: 0.1, rotation: Math.PI / 2, color: '#1a3636' },
        props: { angularVelocity: 0.32, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 160.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 4.0, halfHeight: 0.1, rotation: -Math.PI / 2, color: '#522258' },
        props: { angularVelocity: -0.32, restitution: 0 },
      },

      {
        position: { x: 3.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 135.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 140.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 144.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 147.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 149.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 3.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 4.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 5.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 6.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 7.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 8.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 9.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 10.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 11.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 12.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 13.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 14.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 15.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 16.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 17.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 18.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },
      {
        position: { x: 19.5, y: 150.0 },
        type: 'static',
        shape: { type: 'circle', radius: 0.2, color: '#e6e1ae' },
        props: { restitution: 1.5, destroyOnContact: true },
      },

      {
        position: { x: 4.5, y: 180.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 6.0, halfHeight: 0.1, rotation: 0, color: '#ccb1b1' },
        props: { angularVelocity: 4.6, restitution: 0 },
      },
      {
        position: { x: 18.5, y: 180.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 7.0, halfHeight: 0.1, rotation: 0, color: '#b1ccb1' },
        props: { angularVelocity: -4.0, restitution: 0 },
      },

      {
        position: { x: 11.5, y: 195.0 },
        type: 'kinematic',
        shape: { type: 'box', halfWidth: 8.0, halfHeight: 0.1, rotation: 0, color: '#b3ccff' },
        props: { angularVelocity: 5.5, restitution: 0 },
      },
    ],
  },
];
